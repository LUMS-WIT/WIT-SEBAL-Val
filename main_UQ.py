...
def combine_UQ():
    ...
    for mean_file in MEAN_DIR.glob("*.xlsx"):
        ...
        if not (
            df_mean["Timestamp"].equals(df_lower["Timestamp"]) and
            df_mean["Timestamp"].equals(df_upper["Timestamp"])
        ):
        ...
        df_combined.rename(columns={"sebal_sm": "sebal_sm"}, inplace=True)
